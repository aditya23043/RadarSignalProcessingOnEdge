...
typedef struct {
    ...
    XTrigger_ip_uio_map maps[ MAX_UIO_MAPS ];
} XTrigger_ip_uio_info;

/***************** Variable Definitions **************************************/
static XTrigger_ip_uio_info uio_info;
...
static int uio_info_read_name(XTrigger_ip_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XTrigger_ip_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XTrigger_ip_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XTrigger_ip_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XTrigger_ip_Initialize(XTrigger_ip *InstancePtr, const char* InstanceName) {
	XTrigger_ip_uio_info *InfoPtr = &uio_info;
