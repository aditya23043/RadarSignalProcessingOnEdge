...
struct RET_VALS corr_dopp()/*<<<*/
{
    ...
    for(int i = 0; i < 256; i++)
    {
        ...
        for(int j = 0; j < 512; j++)
        {
            ...
            for(int i = 0; i < 256; i++)
            {
                for(int j = 0; j < 512; j++)
                {
                    printf("{%f, %f}, ", crealf(corr_output_map[j][i]), cimagf(corr_output_map[j][i]));
                }
            }
            printf("\n");
#endif
    ...
    /* >>> */
            ...
            for(int i = 0; i < 256; i++)
            {
                for(int j = 0; j < 512; j++)
                {
                    printf("{%f, %f}, ", crealf(range_dopp_map[j][i]), cimagf(range_dopp_map[j][i]));
                }
            }
            printf("\n");
#endif
    ...
    for (int s = 0; s < NUM_SNR; s++) {
        ...
        for (int mc = 0; mc < NUM_MONTE_CARLO; mc++) {
            ...
            for(int i = 0; i < 256; i++)
            {
                for(int j = 0; j < 1024; j++)
                {
                    printf("{%f, %f}, ", crealf(channel_output[i][j]), cimagf(channel_output[i][j]));
                }
            }
            printf("\n");
#endif
...
#ifdef PRINT_DEBUG
            printf("Trigger IP output: ");
            for(int i = 0; i < 256; i++)
            {
                for(int j = 0; j < 1024; j++)
                {
                    printf("{%f, %f}, ", crealf(trigger_output[i][j]), cimagf(trigger_output[i][j]));
                }
            }
            printf("\n");
#endif
...
#ifdef PRINT_DEBUG
            printf("Trigger IP output (removed cyclic prefix): ");
            for(int i = 0; i < 256; i++)
            {
                for(int j = 0; j < 512; j++)
                {
                    printf("{%f, %f}, ", crealf(trigger_output_512[i][j]), cimagf(trigger_output_512[i][j]));
                }
            }
            printf("\n");
#endif
