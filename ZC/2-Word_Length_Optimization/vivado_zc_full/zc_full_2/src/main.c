...
float complex corr_output_pl[512] __attribute__((aligned(32))) = {0}; // input to doppler ip
float complex corr_output_map[512][256] __attribute__((aligned(32))) = {0}; // input to doppler ip
float complex range_dopp_map[512][256] __attribute__((aligned(32))) = {0}; //output of doppler ip
float complex channel_output[256][512] = {0};
float complex trigger_output[256][1024] __attribute__((aligned(32))) = {0};
float complex trigger_output_512[256][512] __attribute__((aligned(32))) = {0};
...
const int V_OFFSET = 256; // range velocity offset for golay
...
struct RET_VALS corr_dopp()/*<<<*/
{
    ...
    for(int i = 0; i < 256; i++)
    {
        Xil_DCacheFlushRange((UINTPTR)channel_output[i], sizeof(channel_output[i]));
        ...
        /* printf("4\n"); */
        status = XAxiDma_SimpleTransfer(&dma_instance_corr, (UINTPTR)channel_output[i], sizeof(channel_output[i]), XAXIDMA_DMA_TO_DEVICE);
        /* printf("5\n"); */
